...
class MainWindow(Tk):
    # Для реализации создан класс в котором все происходит
    def __init__(self):
        # В функции __init__ описываются только параметры окна TKinter
        Tk.__init__(self)
    ...
    def draw_lines(self):
        # Функция предназначена для работой с кнопкой после ее нажатия.
        # Задает начальные координаты и вызывает рекурсивную функцию
    ...
    def recursion(self, level, x1, y1, x2, y2):
        # Рекурсивная функция рассчитываются новые координаты и строятся линии фигуры
        if level > 1:
